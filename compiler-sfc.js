...
var require$$0 = require('source-map-js');
...
function set(target, key, val) {
    ...
    if (isReadonly(target)) {
        'development' !== 'production' && warn$3(`Set operation on key "${key}" failed: target is readonly.`);
        return;
    }
...
function bindObjectListeners(data, value) {
    if (value) {
        if (!isPlainObject(value)) {
            'development' !== 'production' && warn$3('v-on without argument expects an Object value', this);
        }
        else {
...
const less = (source, map, options) => {
    const nodeLess = require('./less.js');
    let result;
    let error = null;
